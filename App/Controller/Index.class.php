...
class Index extends Common
{
    ...
    public function index()
    {
        ...
        foreach ($categories ?: [] as $k => $c) {
            $categories[$k]['cover_image_url'] = '';
            $cover_image_id = 0;
            if (isset($categories[$k]['cover_image_id']) && $categories[$k]['cover_image_id'] > 0) {
            ...
            //封面图片ID(方便后期扩展)
            $categories[$k]['cover_image_id'] = $cover_image_id ?: 0;
            //封面图片url
            $categories[$k]['cover_image_url'] = $IMAGE->get_category_cover_img_url($cover_image_id, '120d') ?: getImgUrl(0, 90);
        }
